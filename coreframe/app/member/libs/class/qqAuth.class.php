...
class WUZHI_qqAuth {
	...
	public function get_openid($token = ''){
		...
		$response = json_decode(substr(file_get_contents('https://graph.qq.com/oauth2.0/me?access_token='.$token), 9, -3));
	...
	public function get_user_info($openid = ''){
		return json_decode(file_get_contents('https://graph.qq.com/user/get_user_info?oauth_consumer_key='.$this->appid.'&access_token='.$this->token.'&openid='.$this->openid.'&format=json'));
	}
	/**
	 * 得到token
	 */
	public function get_token($code){
		...
		$response = file_get_contents('https://graph.qq.com/oauth2.0/token?grant_type=authorization_code&client_id='.$this->appid.'&client_secret='.$this->appkey.'&code='.$code.'&redirect_uri='.rawurlencode(WEBURL.'index.php?m=member&v=auth&type=qq'));
	...
	public function login(){
		...
		if($r){
			...
			$r['authid'] = $db->insert('member_auth', array('type'=>'qq', 'openid'=>$this->openid, 'token'=>$this->token, 'expires_in'=>$this->expires_in, 'extend'=>array2string($this->extend)), true);
		}
