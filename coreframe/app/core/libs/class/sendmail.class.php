 ...
 * $mail->setServer("smtp.gmail.com", "XXXXX@gmail.com", "XXXXX",'站点名称', 465, true); //设置smtp服务器，到服务器的SSL连接
...
class WUZHI_sendmail {
    ...
    /**
     * @var string 经过base64_encode后的 主题
     * @access protected
     */
    protected $_subject;
    /**
    ...
    /**
     * @var string mail_type
     * @access protected
     */
    protected $_mail_type;

    public function __construct() {
        $configs = get_cache('sendmail');
        $this->setServer($configs['smtp_server'],$configs['smtp_user'],decode($configs['password']),$configs['nickname'],$configs['smtp_port'], $configs['openssl']);
        $this->setFrom($configs['send_email']);
        $this->_sign = $configs['sign'];
        $this->_mail_type = $configs['mail_type'];
    }
    /**
     ...
     * @return boolean
     */
    public function setServer($server, $username = '', $password = '', $nickname = '',$port=25, $isSecurity = 0) {
        ...
        $this->_nickname = $nickname ? $nickname : $this->_nickname;
    ...
    public function setMail($subject, $body) {
        ...
        $this->_body = base64_encode($body.$this->_sign);
        return true;
    }
    ...
    public function sendMail() {
        ...
        if($this->_mail_type==1) {
            mail('', $this->subject, '', $this->_header);
            return TRUE;
        }
    ...
    protected function getCommand() {
        ...
        $header = "FROM: =?".CHARSET."?B?".base64_encode($this->_nickname)."?= <" . $this->_from . ">\r\n";
        ...
        //主题
        $header .= "Subject: =?".CHARSET."?B?" . $this->_subject ."?=\r\n";
        ...
        if(!empty($this->_attachment)){
            ...
            for($i=0; $i<$count; $i++){
                ...
                $header .= "Content-Transfer-Encoding: base64\r\n";
                $header .= 'Content-Disposition: attachment; filename="=?'.CHARSET.'?B?' . base64_encode( basename($this->_attachment[$i]) ) . '?="' . "\r\n";
                $header .= "\r\n";
