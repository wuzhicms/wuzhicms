...
private function images($config, $value){
	...
	if ($value && is_array($value)) {
		foreach ($value AS $k => $v) {
			$default_multiple .= '<li id="file_node_' . $k . '"><input type="hidden" name="form[' . $field . '][' . $k . '][url]" value="' . $v['url'] . '"> <img src="' . $v['url'] . '" alt="' . $v['alt'] . '" onclick="img_view(this.src);"> <textarea name="form[' . $field . '][' . $k . '][alt]" >' . $v['alt'] . '</textarea> <a class="btn btn-danger btn-sm" href="javascript:remove_file(' . $k . ');">移除</a></li>';
		}
	}
