...
function import_sql($id, $weburl)
{
    ...
    $db = $db['default'];
    $link = $is_mysql ? mysql_connect($db['dbhost'], $db['username'], $db['password']) or die ('Not connected : ' . mysql_error()) : mysqli_connect($db['dbhost'], $db['username'], $db['password'], $db['dbname']) or die ('Not connected : ' . mysqli_error());
    $version = $is_mysql ? mysql_get_server_info() : mysqli_get_server_info($link);
    $is_mysql ? mysql_query("SET NAMES '$dbcharset'", $link) : mysqli_query($link, "SET NAMES '$dbcharset'");

    if ($version > '5.0') {
        $is_mysql ? mysql_query("SET sql_mode=''") : mysqli_query($link, "SET sql_mode=''");
    }
    $is_mysql ? mysql_select_db($db['dbname']) : '';
    ...
    case 3:
        if (isset($_POST['admin_username'])) {
            ...
            if($is_mysql){
                ...
                if (!($link = mysqli_connect($dbhost, $username, $password,$dbname))) {
                    exit('2');
                }
...
(1, 0, '" . $configs['admin_username'] . "', '$password', '$factor', 0, '0.00', '0', '" . $configs['admin_email'] . "', 10, 3, 0, 0, 0, 0, '', '127.0.0.1', 0, 0, 0)");
                        mysqli_query($link,"INSERT INTO `" . $db['tablepre'] . "member_detail_data` (`uid`) VALUES ('1')");
                        mysqli_query($link,"INSERT INTO `" . $db['tablepre'] . "admin` (`uid`, `role`, `truename`, `password`, `factor`, `lang`, `department`, `face`, `email`, `tel`, `mobile`, `remark`) VALUES ('1', ',1,', '" . $configs['admin_username'] . "', '$password', '$factor', 'zh-cn', '', '', '', '', '', '')");
