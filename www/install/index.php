			...
			switch($startid) {
				case 1://mysql_config.php
					...
					if($reinstall) {
						...
						if(!file_exists(WWW_ROOT.'configs/'.$reinstall.'mysql_config.php')) {
						...
						if(!file_exists(WWW_ROOT.'configs/'.$reinstall.'route_config.php')) {
							exit('请重新上传文件：'.WWW_ROOT.'configs/'.$reinstall.'route_config.php');
						}
				...
				case 7://开始初始化数据
					...
					if($is_mysql) {
						...
						mysql_query("INSERT INTO `".$db['tablepre']."member` (`uid`, `ucuid`, `username`, `password`, `factor`, `points`, `money`, `mobile`, `email`, `modelid`, `groupid`, `vip`, `viptime`, `islock`, `locktime`, `regip`, `lastip`, `regtime`, `lasttime`, `loginnum`,`ischeck_email`,`ischeck_mobile`) VALUES
(1, 0, '".$configs['admin_username']."', '$password', '$factor', 0, '0.00', '0', '".$configs['admin_email']."', 10, 3, 0, 0, 0, 0, '', '127.0.0.1', 0, 0, 0,1,1)");
						...
						mysqli_query($link,"INSERT INTO `".$db['tablepre']."member` (`uid`, `ucuid`, `username`, `password`, `factor`, `points`, `money`, `mobile`, `email`, `modelid`, `groupid`, `vip`, `viptime`, `islock`, `locktime`, `regip`, `lastip`, `regtime`, `lasttime`, `loginnum`,`ischeck_email`,`ischeck_mobile`) VALUES
(1, 0, '".$configs['admin_username']."', '$password', '$factor', 0, '0.00', '0', '".$configs['admin_email']."', 10, 3, 0, 0, 0, 0, '', '127.0.0.1', 0, 0, 0,1,1)")  or die(mysqli_error($link));
