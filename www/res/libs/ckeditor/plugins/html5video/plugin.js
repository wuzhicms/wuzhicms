CKEDITOR.plugins.add( 'html5video', {
    ...
    init: function( editor ) {
        editor.widgets.add( 'html5video', {
             ...
             */
            allowedContent: 'div[data-responsive](!ckeditor-html5-video){text-align,float,margin-left,margin-right}; video[controls,src,autoplay,width, height]{max-width,height};',
            requiredContent: 'div(ckeditor-html5-video); video[controls,src];',
            ...
            data: function() {
                ...
                if ( this.data.src ) {
                    ...
                    if ( !this.element.getChild( 0 ) ) {
                        ...
                        videoElement.setAttribute( 'class', 'video-js vjs-fluid' );
                        videoElement.setAttribute( 'data-setup', '{}' );
                        // Append it to the container of the plugin.
                        this.element.append( videoElement );
                        
                    }
                    var nowVideo = this.element.getChild( 0 );
					// Create a new <source> element.
					var videoSource = new CKEDITOR.dom.element( 'source' );
					videoSource.setAttribute('src',this.data.src);
					videoSource.setAttribute("type","video/mp4");
					// Append <source> element to the video container.
					nowVideo.append(videoSource);
                    //this.element.getChild( 0 ).setAttribute( 'src', this.data.src );
                    if (this.data.width) nowVideo.setAttribute( 'width', this.data.width );
                    if (this.data.height) nowVideo.setAttribute( 'height', this.data.height );

                    if ( this.data.responsive ) {
                            this.element.setAttribute("data-responsive", this.data.responsive);
                            nowVideo.setStyle( 'max-width', '100%' );
                            nowVideo.setStyle( 'height', 'auto' );
                    } else {
                            nowVideo.removeStyle( 'max-width' );
                            nowVideo.removeStyle( 'height' );
                    }									
                }
