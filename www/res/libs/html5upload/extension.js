...
function callback_more_dialog(filename,htmlid,is_thumb,htmlname)
{
    ...
    $.each( file_array, function(i, n) {
        ...
        str += '<li id="file_node_'+file_id+'" class="d-inline-block me-2 mb-2"><div class="card"><img  class="p-2" src="'+file_url+'" alt="'+file_alt+'" onclick="img_view(this.src);"><input type="hidden" name="'+htmlname+'['+file_id+'][url]" value="'+file_url+'"><div class="card-body pt-0 px-2"><textarea class="form-control" rows="3" name="'+htmlname+'['+file_id+'][alt]" onfocus="if(this.value == this.defaultValue) this.value = \'\'" onblur="if(this.value.replace(\' \',\'\') == \'\') this.value = this.defaultValue;">'+file_alt+'</textarea><a class="btn btn-danger btn-sm btn-xs p-1 w-100 mt-2" href="javascript:remove_file('+file_id+');">移除</a></div></div></li>';
    });
